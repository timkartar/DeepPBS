...
class PPFConv(MessagePassing):
    ...
    def forward(
        ...
        #x = (x[0],torch.zeros_like(x[1])) # set default target features in out to 0
    ...
    def message(self, x_j: OptTensor, x_i: OptTensor, pos_i: Tensor, pos_j: Tensor,
            normal_i: Tensor, normal_j: Tensor) -> Tensor:
        msg = point_pair_features(pos_i, pos_j, normal_i, normal_j)
        if x_j is not None:
            msg = torch.cat([x_j, msg, x_i], dim=1)
